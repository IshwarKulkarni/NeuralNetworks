...
{
    StringUtils::establishEndsWith(Location, "\\");
    ...
    THROW_IF(!allStreamsGood, BadFileStrmException, "All the requisite streams for reading MNIST images/labels are not good");
}
...
unsigned char*** MNISTReader::ImageDataCopy2D(unsigned N, unsigned offset, bool TrainImg)
{
    if (N + offset <= MNISTReader::NumImages)
    {
        ...
        in.seekg(ImageFileOffset + offset* ImageSizeLin);
        in.read(mem[0][0], ImageSizeLin * N);
        THROW_IF(in.gcount() != N * ImageSizeLin, FileIOException, "Did not read enough data");
