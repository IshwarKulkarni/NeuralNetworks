...
#pragma warning(default:4244)
        ...
        T DotAt(Vec::Loc loc, const Matrix<U>& kernel) const
        {
            ...
            start.x = MAX(0, start.x);    start.y = MAX(0, start.y);
            end.x = MIN((int)size.x, end.x); end.y = MIN((int)size.y, end.y);

            T sum = 0; unsigned t = 0;
        ...
        typename std::enable_if< std::is_same<U, uchar>::value||std::is_same<U, __IM_Color__::Color>::value >::type
        WriteAsImage(std::string fileName) const{
            PPMIO::Write(fileName, size.x, size.y , sizeof(T), (uchar*)data[0]);
        }

        inline T& at(int y, int x)
        {
#ifdef _DEBUG
            THROW_IF(x < 0 || y < 0, DataException, "Can't be negative: x = %d, y = %d", x, y);
            THROW_IF(x >= int(size.x) || y >= int(size.y), DataException, "Too Large: x = %d, y = %d", x, y);
#endif
            return data[y][x];
        }
    ...
    struct Matrix3
    {
        ...
        inline Matrix3(Vec::Size3 s = Vec::Zeroes3, T*** d = 0) : size(s), data(d ? d : ColocAlloc<T>(s)) {}

              T& at(Vec::Size3 loc)       { return T[loc.z][loc.y][loc.x]; }
        const T& at(Vec::Size3 loc) const { return T[loc.z][loc.y][loc.x]; }
        ...
        {
#ifdef _DEBUG
            THROW_IF(x < 0 || y < 0 || z < 0, DataException, "Can't be negative: x = %d, y = %d, z = %d", x, y, z);
            THROW_IF(x >= int(size.x) || y >= int(size.y) || z >= int(size.z), DataException, "Too Large: x = %d, y = %d, z = %d", x, y, z);
#endif 
            return data[z][y][x];
        }
        ...
        {
#ifdef _DEBUG
            THROW_IF(z > size.z, DataException, "Frame #z = %d too large", z);
#endif 
            return{ size, data[z]};
        }
    ...
    Out& operator<<(Out& out, const Matrix3<T>& k)
    {
        out << k.size ;
        for (unsigned i = 0; i < k.size.z; ++i)
            OutCSV(out, k.data[i], k.size.x, k.size.y, ("\nFrame[ " + to_string(i) + " ]"  + ",\t").c_str());

        return out;
    } 

    template<typename O, typename T>
    inline void OutCSV(O& outStream, T& data, unsigned w, unsigned h, const char* msg)
    {
        outStream << setprecision(9);
    ...
    inline void Out2d(O& outStream, T& data, unsigned w, unsigned h, const char* msg = "", unsigned fwidth = 7)
    {
        ...
        for (unsigned j = 0; j < w; ++j)
            outStream << setw(fwidth + 1) << setfill(' ') << left << j << " | ";
        ...
        outStream << LogEndl<< dashes << LogEndl;
        for (unsigned i = 0; i < h; ++i)
        {
            outStream << setw(digits + 1) << setfill(' ') << i <<  "|";
            for (unsigned j = 0; j < w ; ++j)
                outStream    << setprecision(fwidth-2) << setw(fwidth + 1) << setfill('0') 
                            << showpoint << left<< fixed 
                            << data[i][j] << " | ";

            outStream << LogEndl;
        }
        outStream << dashes << LogEndl;
    ...
    Matrix<T> ReadCSV(std::istream& strm)
    {
        ...
        THROW_IF(width == 0, UnsupportedFileFormat,
            "File is not organized correctly, number of columns appear to be zero");
    ...
    void ReadCSV(std::istream& strm, unsigned& width, unsigned& height, std::vector<T>& buf)
    {
        ...
        while (strm.good())
        {
            ...
            for (unsigned j = 0; j < width; ++j)
            {
                ...
                c = strm.get();
                THROW_IF(strm.good() && c != ',' && j != width - 1, UnsupportedFileFormat, "Delimiter is invalid");
            }
        }
    }
    ...
    TOuter** Emplace(TInner* inner, Vec::Size2 innerSize, Vec::Size2 outerSize, Vec::Loc loc = LocOrigin)
    {
        assert(innerSize + loc > outerSize, "Dimension failure");
        T** outer = ColocAlloc(outerSize);
        for (unsigned i = loc.y; i < loc.y + inner.y; ++i)
            std::copy(outer[i], outer[i + innerSize.x], inner[i - loc.y]);
    }

    template<typename TInner, typename TOuter>
    void Emplace(Matrix<TInner>& inner, Matrix<TInner>& outer, std::unary_function<TInner, TOuter>& conv, Vec::Loc loc = LocOrigin)
    {
        ASSERT(inner.size + loc > outer.size, "Dimernsion failure");
        for (unsigned i = loc.y; i < loc.y + inner.y; ++i)
            std::transform(outer[i], outer[i + innerSize.x], inner[i - loc.y], conv);
    }
    ...
    static bool MakeFilters()
    {
        ...
        for (unsigned i = 0; i < SobelV.size(); ++i)
            SobelV[0][i] = sobelV[i], SobelH[0][i] = sobelH[i];
